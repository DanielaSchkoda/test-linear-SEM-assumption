...
graphics_dir = "C:\\Users\\Annika\\OneDrive\\Dokumente\\Masterarbeit\\00_Eigentliche Arbeit\\Graphics\\"
...
```{r}
write.csv(results, file="results_tuebingen_pairs.csv",row.names=F)
results <- read.csv("results_tuebingen_pairs.csv")
```
...
results$accepted_u <- apply(results, 1, function(x) if (x[["pval_full_u"]]<=0.05 && x[["pval_both_DAGs_u"]]<=0.05) "rejected by both" else if(x[["pval_full_u"]]<=0.05) "rejected by full graph test" else if(x[["pval_both_DAGs_u"]]<=0.05) "rejected by DAG test" else "accepted")

results$accepted_indep <- apply(results, 1, function(x) if (x[["pval_indep_full_graph"]]<=0.05 && x[["pval_both_DAGs_indep"]]<=0.05) "rejected by both" else if(x[["pval_indep_full_graph"]]<=0.05) "rejected by full graph test" else if(x[["pval_both_DAGs_indep"]]<=0.05) "rejected by DAG test" else "accepted")

results$accepted_KP <- apply(results, 1, function(x) if (x[["pval_KP"]] <= 0.05) "rejected" else "accepted")

threshold <- 0.05 * 0.9
results$accepted_bootstrap <- apply(results, 1, function(x) if (x[["r_0"]] == 1 && x[["pval_boot"]] > threshold) "rank 1" else if (x[["r_0"]] == 2 && x[["pval_boot"]] > threshold) "rank 2" else "rejected")
...
for (test in c("indep", "u", "bootstrap", "KP")) {
  ...
  ggsave(paste(graphics_dir, "tueb_results_", test, ".png", sep=""), width = 35, height = 31, units = "cm")
}
```

## Plot Distributions Pairs
...
for (i in 5:17) {
  ...
  ggsave(paste(graphics_dir, "tueb_", i, "_distr_V1.png", sep=""), width = 12, height = 10, units = "cm")
  ...
  range_V2 = max(cause_effect_pair$V2) - min(cause_effect_pair$V2)
    ...
    geom_density() +
    labs(title="",x="Hourly Wage", y = "")+
    theme(plot.title = element_text(face="bold")) + 
    theme(plot.title = element_text(hjust = 0.5))
      
  ggsave(paste(graphics_dir, "tueb_", i, "_distr_V2.png", sep=""), width = 12, height = 10, units = "cm")

  ggplot(cause_effect_pair, aes(x=V1, y=V2)) +
    geom_point(shape=19, size=1) +
    labs(title="",x="V1", y = "V2")

  ggsave(paste(graphics_dir, "tueb_", i, "_scatter.png", sep=""), width = 12, height = 10, units = "cm")
}
```
