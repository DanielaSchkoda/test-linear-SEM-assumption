...
class Monomial {
  public:
    double coef;
    // second_moms has 2 cols, as many rows as second moms occur
    IntegerMatrix second_moms;
    // 3 cols
    IntegerMatrix third_moms;
    int degree;

    // constructor
    Monomial(double coef_in, IntegerMatrix second_moms_in, IntegerMatrix third_moms_in){
      coef = coef_in;
      second_moms = second_moms_in;
      third_moms = third_moms_in;
      degree = second_moms_in.nrow() + third_moms_in.nrow();
    }; 
  ...
  public:
    ...
    double estimate_poly(NumericMatrix L){
      ...
      for(int i=0; i < monomials.size(); i++){ 
        ...
        IntegerMatrix second_moms = current_mon.second_moms - 1;
        IntegerMatrix third_moms = current_mon.third_moms - 1;
        int m_deg = 0;
        for(int j=0; j < second_moms.nrow(); j++){
          monomial_est = monomial_est * L(m_deg, second_moms(j, 0)) * L(m_deg, second_moms(j, 1));
          m_deg++;
        }
        for(int j=0; j < third_moms.nrow(); j++){
          monomial_est = monomial_est * L(m_deg, third_moms(j, 0)) * L(m_deg, third_moms(j, 1)) * L(m_deg, third_moms(j, 2));
          m_deg++;
        }
...
// a vector of the form c(coef=1, second_moms=matrix(c(1, 2), 1, 2), third_moms=matrix(c(1, 2, 1), 1, 3))
// second_moms needs to have 2 columns, third_moms 3 columns. Each row represents one
// second respectively third moment appearing in the monomial.
Polynomial create_c_polynomial(List R_poly){
  ...
  for (int j = 0; j < R_poly.length(); j++){
    ...
    IntegerMatrix second_moms = as<IntegerMatrix>(R_mon["second_moms"]);
    IntegerMatrix third_moms = as<IntegerMatrix>(R_mon["third_moms"]);
    
    C_monomials.push_back(Monomial(coef, second_moms, third_moms));
  }
  return(C_monomials);
}
